...
const initialSongs: Song[] = [
    { id: 1, title: "Starboy", artist: "The Weeknd, Daft Punk", duration: "03:50", cover: "/images/music/starboy.svg" },
    { id: 2, title: "Disaster", artist: "Conan Gray", duration: "03:58", cover: "/images/music/disaster.svg" },
    { id: 3, title: "HANDSOME", artist: "Warren Hue", duration: "04:45", cover: "/images/music/handsome.svg" },
    { id: 4, title: "Sharks", artist: "Imagine Dragons", duration: "05:23", cover: "/images/music/sharks.svg" },
    { id: 5, title: "Fly Me To The Sun", artist: "Romantic Echoes", duration: "04:20", cover: "/images/music/thesun.svg" },
    { id: 6, title: "The Bended Man", artist: "Sunwich", duration: "03:48", cover: "/images/music/theman.svg" },
    { id: 7, title: "Somebody's Nobody", artist: "Alexander 23", duration: "03:57", cover: "/images/music/nobody.svg" },
];
...
export default function BasicTableOne() {
    ...
    // helper to read a File into a data URL so it can be persisted in localStorage
    ...
    // localStorage keys
    const SONGS_LS_KEY = 'md_songs_v1';
    const PLAYLISTS_LS_KEY = 'md_playlists_v1';

    // load persisted data on mount
    React.useEffect(() => {
        try {
            const raw = localStorage.getItem(SONGS_LS_KEY);
            if (raw) {
                const parsed = JSON.parse(raw) as Song[];
                if (Array.isArray(parsed) && parsed.length) setSongs(parsed);
            }
        } catch (e) {
            // ignore parse errors and keep initialSongs
        }
        ...
        try {
            ...
            if (raw) {
                ...
                if (Array.isArray(parsed) && parsed.length) setPlaylists(parsed as any[]);
            }
        } catch (e) {}
    }, []);

    // persist songs/playlists when they change
    React.useEffect(() => {
        try {
            localStorage.setItem(SONGS_LS_KEY, JSON.stringify(songs));
        } catch (e) {}
    }, [songs]);
    ...
    const handleDeleteSelectedPlaylists = () => {
        if (!selectedPlaylistIds || selectedPlaylistIds.size === 0) return;
        ...
        <div className="w-full min-h-screen px-6 py-6">
            <div className="mb-4">
                ...
                <div className="mt-4 flex items-center justify-between">
                    <div className="text-gray-800 font-semibold">{activeTab === 'media' ? '560 songs' : '68 Play list'}</div>
            ...
            Filter
                        ...
                        <button onClick={() => setIsAddOpen(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-md text-sm">+ Add new</button>
                    </div>
                </div>
            </div>
                                ...
                                <div key={song.id} className="flex items-center justify-between px-6 py-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-gray-500 text-xs mt-1">{song.artist} &nbsp;|&nbsp; {song.duration} mins</div>
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-4">
                                        <button aria-label="play" className="w-9 h-9 flex items-center justify-center rounded-full bg-orange-500 text-white shadow">
                            ...
                            <Link href={`/media/playlist/${item.id}`} >
                                <div className="p-4">
                                    ...
                                    <div className="mt-4 text-left">
                                        ...
                                        <p className="text-xs text-gray-500">{item.artist} &nbsp;|&nbsp; {item.year}</p>
                                ...
                                <div className="border-t border-gray-100" />
                                                    ...
                                                    {/* checkbox for selecting playlists for bulk delete */}
                ...
                <div className="fixed inset-0 z-50 flex">
                    ...
                    <aside className="relative ml-auto w-full max-w-[560px] h-screen bg-white rounded-l-2xl shadow-xl flex flex-col">
                        <div className="p-6 overflow-y-auto flex-1">
                            <button onClick={() => setIsAddOpen(false)} aria-label="close" className="absolute right-6 top-6 w-10 h-10 flex items-center justify-center rounded-full bg-white shadow-md">
                            ...
                            <h3 className="text-xl font-semibold text-gray-800 mb-1">{activeTab === 'playlist' ? 'Add new play list' : 'Add new media'}</h3>
                            <p className="text-sm text-gray-500 mb-6">{activeTab === 'playlist' ? 'Update your play list details.' : 'Update your media details.'}</p>
                                ...
                                {activeTab === 'playlist' ? (
                                    <>
                                        <label className="flex flex-col">
                                            ...
                                            <input value={playlistName} onChange={(e) => setPlaylistName(e.target.value)} className="w-full border border-gray-200 rounded-md px-4 py-3 text-sm" />
                                        </label>
                                        ...
                                        <div>
                                            ...
                                            <div className="rounded-lg border border-dashed border-gray-200 p-4">
                                                <div className="max-h-[300px] p-3">
                                                    <DropzoneComponent onFileChange={async (file: File) => {
                                                        try {
                                    ...
                                    <>
                                        <label className="flex flex-col">
                                            ...
                                            <input value={mediaName} onChange={(e) => setMediaName(e.target.value)} className="w-full border border-gray-200 rounded-md px-4 py-3 text-sm" />
                                        </label>

                                        <label className="flex flex-col">
                                            <span className="text-sm text-gray-600 mb-2">Link</span>
                                            <input value={mediaLink} onChange={(e) => setMediaLink(e.target.value)} className="w-full border border-gray-200 rounded-md px-4 py-3 text-sm" />
                                        </label>

                                        <label className="flex flex-col">
                                            <span className="text-sm text-gray-600 mb-2">Playlist</span>
                                            <select value={mediaPlaylist} onChange={(e) => setMediaPlaylist(e.target.value)} className="w-full border border-gray-200 rounded-md px-4 py-3 text-sm">
                        ...
                        <div className="border-t border-gray-100 p-4 flex justify-end gap-3">
                            <button onClick={() => { resetForm(); setIsAddOpen(false); }} className="px-4 py-2 rounded-md border border-gray-200 bg-white">Close</button>
                            {activeTab === 'playlist' ? (
                                <button onClick={handleSavePlaylist} className="px-4 py-2 bg-indigo-600 text-white rounded-md">Add new</button>
                            ) : (
